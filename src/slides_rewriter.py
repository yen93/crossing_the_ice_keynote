...
REWRITE_TOOL = {
    ...
    "function": {
        ...
        "description": "Rewritten text for each editable shape in the Crossing the Ice Keynote proposal deck, tailored to the new client.",
        "parameters": {
            "type": "object",
            "properties": {
                "shapes": {
        ...
        messages=[
            {
                "role": "user",
                "content": (
                    "This is the master Crossing the Ice Keynote proposal deck for James "
                    "Castrission's 'Crossing the Ice' keynote — a real, previously-used "
                    "proposal, not a blank template. It may carry a leftover mention of "
                    "whichever client it was last used for (an organisation name inside a "
                    "sentence, e.g. '...at your next X event') and/or a client-specific "
                    "event date on the cover slide. The client's logo image is handled "
                    "separately outside this text rewrite — ignore any image shapes "
                    "entirely.\n\n"
                    "YOUR ONLY JOB: find any shape that mentions a leftover client/"
                    "organisation name (a reference to whoever this deck was last sent to) "
                    "and replace that name with the new client's organisation name from the "
                    "demo notes below, making only the minimal wording change needed for "
                    "the sentence to still read naturally. If the cover slide's date line "
                    "clearly refers to a specific past client's event date rather than a "
                    "generic placeholder, and a real event_date was extracted from the demo "
                    "notes, update it to that new date — otherwise leave it unchanged.\n\n"
                    "CRITICAL — every other shape must be returned COMPLETELY UNCHANGED, "
                    "character-for-character identical to its original text: the speaker's "
                    "bio and credentials, the cover title/tagline, the 'Crossing the Ice' "
                    "expedition story, the Learning Outcomes list, the client testimonials, "
                    "the Conference Gifts paragraph, the DESIGN/DELIVERY inclusions, the "
                    "investment/pricing figures, and the contact/sign-off block are all "
                    ...
                    "structure stay exactly as they are. Do not invent a different price, "
                    "and do not add sentences about the event date/venue/audience beyond "
                    "what's described above.\n\n"
                    "Return every shape passed to you, including the unchanged ones, with "
                    "new_text set to their original text verbatim if no change applies.\n\n"
