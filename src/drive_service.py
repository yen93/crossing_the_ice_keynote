...
def get_master_template(drive) -> dict:
    """Crossing the Ice Keynote has a single offering/template (unlike
    Uncharted Ice's multi-template LLM selector) — this just finds the one
    file in the templates folder whose name contains "template"
    (case-insensitive) and returns {id, title}. Raises if none or more than
    one is found, since an ambiguous match here should stop the run rather
    than guess."""
    resp = (
        drive.files()
        .list(
    ...
    if len(candidates) > 1:
        ...
        raise RuntimeError(
            f"Expected exactly one Crossing the Ice Keynote master template, found {len(candidates)}: {names}"
        )
