"""Orchestrates the Crossing the Ice Keynote proposal pipeline for every
unprocessed matching email found on a single run."""
...
def process_email(clients: GoogleClients, supabase, message_id: str, attachment: dict) -> None:
    ...
    # complete first to know it. No notification thread exists yet at this
    # point, so a failure here needs its own handling rather than falling
    # into the try block below.
    try:
        ...
        log.exception("Download/OCR failed for message %s", message_id)
        try:
            gmail_service.start_notification_thread(
                gmail,
                subject="Automation: Crossing the Ice Keynote Proposal — processing failed",
                body_text=f"Could not process message {message_id}: {exc}",
            )
        except Exception:
            log.exception("Failed to send download/OCR failure notification for message %s", message_id)
    ...
    subject = f"Automation: Crossing the Ice Keynote Proposal for {client_org}"

    thread = gmail_service.start_notification_thread(
        gmail,
        subject=subject,
        body_text=(
            f"Automated Crossing the Ice Keynote proposal creation for {client_org} has "
            "been started. You will be notified through this same email thread once done."
        ),
    )
    ...
    try:
        ...
        ready_message = f"The Crossing the Ice Keynote proposal for {client_org} is ready: {duplicate['view_url']}"
        if match_notes:
